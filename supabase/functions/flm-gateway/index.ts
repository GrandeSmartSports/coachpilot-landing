// Field Command gateway (v10). Deployed to the CoachPilot Supabase project
  ...
  try {
    ...
    if (action === "state") {
      ...
      (settings.data ?? []).forEach((r: { key: string; value: string }) => {
        if (r.key !== "admin_pin") s[r.key] = r.value;
      });
      return json({ ok: true, settings: s, seasons: seasons.data, fields: fields.data, teams: teams.data, slots: slots.data, announcements: announcements.data, games: games.data, ext_teams: extTeams.data });
    }
